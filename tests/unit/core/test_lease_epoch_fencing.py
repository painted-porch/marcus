...
class TestSameAgentContinuationKeepsItsEpoch:
    """Re-claiming a task you already hold is a continuation, not a new claim.

    ``report_task_progress`` re-grants a lease whose grace has elapsed but
    which the monitor has not yet recovered — a window of up to one
    monitor interval. Rotating the epoch there is silently fatal: the
    agent is told its epoch exactly once, by ``request_next_task``, so it
    keeps sending the old value and is fenced on its own completion.
    """

    @pytest.mark.asyncio
    async def test_same_agent_reclaim_keeps_the_epoch(self, lease_manager) -> None:
        """A second create_lease for the current holder does not rotate."""
        first = await lease_manager.create_lease("task-1", "agent-1", _make_task())

        second = await lease_manager.create_lease("task-1", "agent-1", _make_task())

        assert second.lease_epoch == first.lease_epoch

    @pytest.mark.asyncio
    async def test_a_different_agent_still_gets_a_new_epoch(
        self, lease_manager
    ) -> None:
        """The fence must still fire for a genuine handoff."""
        first = await lease_manager.create_lease("task-1", "agent-1", _make_task())

        second = await lease_manager.create_lease("task-1", "agent-2", _make_task())

        assert second.lease_epoch > first.lease_epoch

    @pytest.mark.asyncio
    async def test_reclaim_after_release_still_rotates(self, lease_manager) -> None:
        """Continuation means *currently holding* — not merely 'held before'.

        Once the lease is released the agent has genuinely lost the card,
        so re-claiming it is a new claim and must take a new epoch.
        """
        first = await lease_manager.create_lease("task-1", "agent-1", _make_task())
...
class TestEpochSurvivesRecoveryAcrossRestart:
    """The high-water mark must outlive the assignment row it came from.

    ``recover_expired_lease`` drops the lease **and** calls
    ``remove_assignment``. Seeding ``_task_epochs`` only from surviving
    assignment rows therefore loses the mark for exactly the tasks that
    have been recovered — and a recovered task is the only kind that ever
    has a displaced agent. After a restart the next claimant was reissued
    epoch 1, which compares equal to the displaced agent's token, so the
    fence failed **open** in precisely the scenario D11 exists for.
    """

    @pytest.mark.asyncio
    async def test_epoch_high_water_is_persisted_independently(
        self, lease_manager, mock_persistence
    ) -> None:
        """Claiming a task records its epoch outside the assignment row."""
        await lease_manager.create_lease("task-1", "agent-1", _make_task())

        mock_persistence.record_task_epoch.assert_awaited()
        args = mock_persistence.record_task_epoch.await_args.args
        assert args[0] == "task-1"
        assert args[1] == 1

    @pytest.mark.asyncio
    async def test_reclaim_after_recovery_and_restart_rotates(
        self, mock_kanban_client, tmp_path
    ) -> None:
        """The displaced agent's token stays fenced across a restart.

        Drives the real persistence layer: claim, recover (which removes
        the assignment row), restart, re-claim. The replacement must not
        be handed the epoch the displaced agent is still carrying.
        """
        from src.core.assignment_persistence import AssignmentPersistence

        storage = tmp_path / "assignments"
        first = AssignmentLeaseManager(
            mock_kanban_client, AssignmentPersistence(storage)
        )
        lease = await first.create_lease("task-1", "agent-a", _make_task())

        # Recovery removes both the lease and the assignment row.
        first.active_leases.pop("task-1", None)
        await first.assignment_persistence.remove_assignment("agent-a")

        second = AssignmentLeaseManager(
            mock_kanban_client, AssignmentPersistence(storage)
        )
        await second.load_active_leases()
        replacement = await second.create_lease("task-1", "agent-b", _make_task())

        assert replacement.lease_epoch > lease.lease_epoch
